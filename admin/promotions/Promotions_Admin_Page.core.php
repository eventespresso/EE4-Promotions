...
class Promotions_Admin_Page extends EE_Admin_Page
{
    ...
    {
        $pro_id             =
            ! empty($this->_req_data['PRO_ID']) && ! is_array($this->_req_data['PRO_ID']) ? $this->_req_data['PRO_ID']
                : 0;
        $this->_page_routes = [
            'default'             => [
                'func'       => '_list_table',
    ...
    protected function _list_table()
    {
        ...
        if (
            EE_Registry::instance()->CAP->current_user_can(
                'ee_edit_promotions',
                'esspresso_promotions_create_new_promotion'
            )
        ) {
    ...
    protected function _set_promotion_object()
    {
        ...
        $this->_promotion =
            ! empty($this->_req_data['PRO_ID']) ? EEM_Promotion::instance()->get_one_by_ID($this->_req_data['PRO_ID'])
    ...
    protected function _insert_update_promotions($new = false)
    {
        ...
        if (empty($promotion_values['PRO_ID'])) {
            $promotion = EE_Promotion::new_instance($promotion_values, null, ['Y-m-d', 'g:i a']);
        } else {
    ...
    protected function _duplicate_promotion()
    {
        ...
        if ($success) {
            EE_Error::add_success(
                __('Promotion successfully duplicated, make any additional edits and update.', 'event_espresso')
            );
        }
    ...
    protected function _trash_or_restore_promotion($trash = false, $PRO_ID = 0, $redirect = true)
    {
        ...
        if (empty($PRO_ID)) {
            ...
            if ($trash) {
                EE_Error::add_error(
                    __('Unable to trash a promotion because no promotion id is accessible.', 'event_espresso'),
                    __FILE__,
                    __FUNCTION__,
                    __LINE__
                );
                ...
                EE_Error::add_error(
                    __('Unable to restore a promotion because no promotion id is accessible.', 'event_espresso'),
                    __FILE__,
                    __FUNCTION__,
                    __LINE__
                );
        ...
        if ($success) {
            $success = $trash
                ? EEM_Promotion::instance()->delete_by_ID($PRO_ID) : EEM_Promotion::instance()->restore_by_ID(
                    $PRO_ID
                );
        }
        ...
        if ($success) {
            ...
            $trash
                ? EE_Error::add_error(
                __('Promotion did not get trashed.', 'event_espresso'),
                __FILE__,
                __FUNCTION__,
                __LINE__
            )
                : EE_Error::add_error(
                __('Promotion did not get restored.', 'event_espresso'),
                __FILE__,
                __FUNCTION__,
                __LINE__
            );
    ...
    protected function _trash_or_restore_promotions($trash = false)
    {
        ...
        if (! empty($pro_ids)) {
            ...
            $trash
                ? EE_Error::add_error(
                __('Unable to trash promotions because none were selected.', 'event_espresso'),
                __FILE__,
                __FUNCTION__,
                __LINE__
            )
                : EE_Error::add_error(
                __('Unable to restore promotions because none were selected.', 'event_espresso'),
                __FILE__,
                __FUNCTION__,
                __LINE__
            );
        ...
        if ($success) {
            ...
            if ($promotion instanceof EE_Promotion) {
                ...
                if ($success) {
                    ...
                    EE_Error::add_error(
                        sprintf(
                            __('Unable to permanently delete the %s promotion.', 'event_espresso'),
                            $promotion->name()
                        ),
                        __FILE__,
                        __FUNCTION__,
    ...
    protected function _delete_promotions()
    {
        ...
        if (! empty($pro_ids)) {
            ...
            $success = false;
            EE_Error::add_error(
                __('Unable to permanently delete any promotions because none were selected.', 'event_espresso'),
                __FILE__,
                __FUNCTION__,
                __LINE__
            );
                ...
                'subsections'     => apply_filters(
                    ...
                    [
                        ...
                        'banner_template'     => new EE_Select_Input(
                            ...
                            [
                                'default'         => isset($this->_config->banner_template)
                        ...
                        'ribbon_banner_color' => new EE_Select_Input(
                            ...
                            [
                                'default'         => isset($this->_config->ribbon_banner_color)
                                    ? $this->_config->ribbon_banner_color : false,
